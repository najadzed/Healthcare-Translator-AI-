...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 2. Environment Variable & Parameter Check
        if (!process.env.GOOGLE_API_KEY) {
            // This is critical on Vercel
            return res.status(500).json({ error: 'Missing GOOGLE_API_KEY. Please set the environment variable in Vercel settings.' });
        }
        ...
        // --- Actual Integration with Google GenAI (Gemini) ---
        
        // Initialize the Google GenAI client
        const ai = new GoogleGenAI({ apiKey: process.env.GOOGLE_API_KEY });

        // Define the system prompt for professional medical translation
        const systemInstruction = `You are a professional medical translator. Translate the following text from ${sourceLang || 'auto-detected language'} to ${targetLang}. Prefer patient-friendly wording while keeping medical precision. ${refine ? 'Also fix obvious ASR errors, abbreviations, and misspellings commonly heard in medical speech.' : ''}

        Return ONLY the translated text and nothing else (no introductory phrases, no explanations, no formatting, etc.).`;
        ...
        const response = await ai.models.generateContent({
            model: "gemini-2.5-flash", // A fast and capable model for translation tasks
            contents: [
                { role: "user", parts: [{ text: userPrompt }] }
            ],
            config: {
                systemInstruction: systemInstruction,
                // Set temperature low for precise, literal translation
                temperature: 0.1, 
            }
        });
        ...
        // Simple cleanup of potential unwanted quotes/wrappers from the model
        if (translated.startsWith('"""') && translated.endsWith('"""')) {
            translated = translated.slice(3, -3).trim();
        ...
        // Catch network, authentication, or other API errors
        return res.status(500).json({ 
            error: 'Server error during translation API call',
            details: e.message 
        });
    }
}